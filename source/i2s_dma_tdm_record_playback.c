...
int main(void)
{
    ...
    while (1)
    {
        if (emptyBlock > 0)
        {
            xfer.data     = Buffer + rx_index * BUFFER_SIZE; // wskaznik na miejsce w buforze do zapisu
        ...
        {
            xfer.data     = Buffer + tx_index * BUFFER_SIZE; // wskaznik na miejsce w buforze do odczytu
            xfer.dataSize = BUFFER_SIZE;
